...
// --- PDF Generation Logic is now inside this file ---
  ...
  try {
    ...
    autoTable(doc, {
      ...
      didDrawPage: (data) => {
        ...
        doc.text("Product Inventory Report", 38, 25);
        doc.setDrawColor(224, 224, 224);
        doc.line(14, 35, pageWidth - 14, 35);

        // --- Summary Section (only on the first page) ---
        if (data.pageNumber === 1) {
          doc.setFontSize(12);
          doc.setFont("helvetica", "bold");
          doc.text("Report Summary", 14, data.cursor.y + 10); // Position dynamically
          autoTable(doc, {
            body: [
              ...
              ["Unavailable Products:", unavailableCount],
            ],
          ...
          // Set the startY for the main table after this summary
          data.settings.startY = doc.lastAutoTable.finalY + 10;
        }
        ...
        const footerText = `Page ${
          data.pageNumber
        } of ${doc.internal.getNumberOfPages()} | Generated on: ${new Date().toLocaleString()}`;
    ...
    doc.save(`${brandingData.name}_Product_Report.pdf`);
    return { success: true };
  } catch (err) {
